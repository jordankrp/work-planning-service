...
options_menu = [
    "Display all workers",
    "Display specific worker",
    "Add new worker",
    "Add / edit worker shift",
    "Delete shift",
    "Delete worker",
]
...
def get_worker_id():
    ...
    answer = prompt({"type": "input", "name": "worker_id", "message": "Worker ID:"})
...
def get_new_worker_details():
    ...
    print()
    answer.update(prompt({"type": "input", "name": "name", "message": "Worker name:"}))
    print()
    print(term.underline("Please type in the number of shifts"))
    print()
    answer.update(
        prompt(
            {"type": "input", "name": "number_shifts", "message": "Number of shifts:"}
        )
    )
    ...
    for i in range(number_shifts):
        print(
            term.underline(
                f"Please type in the date of shift {i+1} in the format DD-MM-YYYY."
            )
        )
        print()
        answer.update(
        ...
        print(
            term.underline(
                f"Please type in the time of shift {i+1}. Accepted answers only 0-8, 8-16, 16-24."
            )
        )
        print()
        answer.update(
...
def get_new_shift():
    ...
    print()
    answer.update(
        prompt({"type": "input", "name": "shift_date", "message": "Shift date:"})
    )
    print()
    print(term.underline("Please type in the worker shift time"))
    print()
    answer.update(
        prompt({"type": "input", "name": "shift_time", "message": "Shift time:"})
    )
    print()

    return (
        str(answer["worker_id"]),
        str(answer["shift_date"]),
        str(answer["shift_time"]),
    )
